            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ $courseGrade['course']->title }}</h3>
                        <a href="{{ route('student.grades.course', $courseGrade['course']) }}" class="text-sm text-amber-600 dark:text-amber-400 hover:underline">Lihat Detail</a>
                    </div>
                    
                    <!-- Assignment Grades -->
                    @if(count($courseGrade['assignments']) > 0)
                        <div class="mb-6">
                            <h4 class="text-md font-medium text-gray-900 dark:text-gray-100 mb-3">Nilai Tugas</h4>
                                        ...
                                        @foreach($courseGrade['assignments'] as $assignment)
                                            <tr>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $assignment['assignment']->title }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    @if(count($courseGrade['quizzes']) > 0)
                        <div>
                            <h4 class="text-md font-medium text-gray-900 dark:text-gray-100 mb-3">Nilai Kuis & Test</h4>
                            
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead class="bg-gray-50 dark:bg-gray-700">
                                        <tr>
                                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Kuis</th>
                                    ...
                                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        @foreach($courseGrade['quizzes'] as $quiz)
                                            <tr>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $quiz['quiz']->title }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
